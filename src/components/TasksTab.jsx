        ...
        <div className="space-y-6">
            ...
            {role === 'teacher' && !isFormExpanded && (
                <div 
                    ...
                    className={`${glassCard} !p-4 mb-6 transition-all duration-300 hover:shadow-xl cursor-pointer border border-gray-200/90 dark:border-gray-700/90 rounded-2xl group hover:border-[#AD3333]/30 dark:hover:border-red-500/30`}
                >
                    <div className="flex items-center gap-3">
                        {teacherPic ? (
                            <img 
                                src={teacherPic} 
                                alt={loggedInName} 
                                className="w-11 h-11 rounded-full object-cover shadow-md border-2 border-white dark:border-gray-700 shrink-0 group-hover:scale-105 transition-transform" 
                            />
                        ) : (
                            <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#AD3333] to-[#8a2828] text-white flex items-center justify-center font-bold text-sm shadow-md shrink-0 border-2 border-white dark:border-gray-700 group-hover:scale-105 transition-transform">
                                {loggedInName?.charAt(0) || 'G'}
                            </div>
                        )}
                        <div className={`flex-1 px-4 py-2.5 rounded-full border text-sm font-medium transition-all ${isDarkMode ? 'bg-gray-800/80 border-gray-700 text-gray-300 group-hover:bg-gray-800 group-hover:border-gray-600' : 'bg-gray-100/80 border-gray-200 text-gray-600 group-hover:bg-gray-100 group-hover:border-gray-300'}`}>
                            ¿Qué deseas asignar o publicar hoy, {loggedInName?.split(' ')[0]}?
                        </div>
                        <button type="button" className="p-2.5 rounded-full bg-red-50 dark:bg-red-950/40 text-[#AD3333] group-hover:scale-110 group-hover:bg-[#AD3333] group-hover:text-white transition-all shadow-sm" title="Crear">
                            <Plus size={18} />
                        </button>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 mt-3 pt-3 border-t border-gray-200/60 dark:border-gray-700/60 text-xs font-semibold text-gray-600 dark:text-gray-400">
                        <div className="flex gap-2">
                            <button 
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50/60 dark:bg-red-950/30 text-[#AD3333] hover:bg-red-100 dark:hover:bg-red-950/60 transition-colors"
                            >
                            ...
                            <button 
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50/60 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-950/60 transition-colors"
                            >
                            ...
                            <button 
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100/60 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                            >
                                <PaperclipIcon size={15} /> Adjuntar
                            </button>
                        </div>
                        <div className="flex items-center gap-1.5 text-purple-600 dark:text-purple-400 text-xs font-bold bg-purple-50 dark:bg-purple-950/30 px-2.5 py-1 rounded-full border border-purple-200/50 dark:border-purple-800/40">
            ...
            {/* VISTA EXPANDIDA (FORMULARIO ULTRA MODERNO CON ANIMACIÓN FLUIDA) */}
            {role === 'teacher' && isFormExpanded && (
                <div className={`${glassCard} !p-6 mb-6 flex flex-col gap-4 border border-gray-300/90 dark:border-gray-700 shadow-2xl rounded-3xl animate-in fade-in slide-in-from-top-4 zoom-in-98 duration-300 origin-top`}>
                    {/* Cabecera del formulario */}
                    <div className="flex justify-between items-center pb-3 border-b border-gray-200/80 dark:border-gray-700/80">
                        <div className="flex items-center gap-3">
                            {teacherPic ? (
                                <img 
                                    src={teacherPic} 
                            ...
                            <div>
                                <h3 className={`font-bold text-sm leading-tight ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>{loggedInName}</h3>
                                <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Crear contenido docente</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl border border-gray-200 dark:border-gray-700">
                                <button 
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${postType === 'task' ? 'bg-white dark:bg-gray-900 shadow-sm text-[#AD3333]' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900'}`}
                                >
                                ...
                                <button 
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${postType === 'post' ? 'bg-white dark:bg-gray-900 shadow-sm text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900'}`}
                                >
                    ...
                    {/* Selector interactivo de grupo / materia en formato chips */}
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2 items-center">
                            ...
                            {academicGroups?.length === 0 && (
                                <span className="text-xs text-gray-500 italic">Crea materias en el Directorio para asignarlas individualmente.</span>
                            )}
                        </div>
                    </div>

                    {/* Editor unificado moderno */}
                    <div className={`p-4 rounded-2xl border transition-all ${isDarkMode ? 'bg-gray-850/60 border-gray-700/80 focus-within:border-blue-500' : 'bg-gray-50/80 border-gray-200 focus-within:border-blue-400 focus-within:bg-white'}`}>
                        <input 
                            value={taskTitle} 
                            onChange={(e) => setTaskTitle(e.target.value)} 
                            placeholder={postType === 'task' ? "Título claro de la tarea..." : "Título de la publicación..."} 
                            className={`w-full text-base font-bold bg-transparent outline-none placeholder:text-gray-400 pb-2 border-b ${isDarkMode ? 'text-gray-100 border-gray-700' : 'text-gray-800 border-gray-200'}`} 
                            required 
                        />
                        <textarea 
                            value={taskDesc} 
                            ...
                            className={`w-full text-sm bg-transparent outline-none placeholder:text-gray-400 pt-3 min-h-[95px] resize-y leading-relaxed ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`} 
                            required 
                        />

                        {/* Previsualización de adjuntos multimedia */}
                        {(showImageInput || postImageUrl || postFileUrl || audioPub) && (
                            <div className="pt-3 border-t border-gray-200/60 dark:border-gray-700/60 space-y-2 animate-in fade-in">
                                {showImageInput && (
                                ...
                                {postFileUrl && (
                                    <div className="flex items-center gap-2 p-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 text-xs font-bold text-blue-700 dark:text-blue-300 w-fit">
                                ...
                                {audioPub && (
                                    <div className="flex items-center gap-2 p-2 rounded-xl bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 w-fit">
                        ...
                        {upPub && <p className="text-xs text-purple-600 italic pt-2">Subiendo audio adjunto...</p>}
                    </div>

                    {/* Barra de herramientas inteligentes de IA y adjuntos */}
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button 
                                ...
                                disabled={isAiLoading} 
                                className={`px-3 py-2 rounded-xl text-xs font-bold border transition-all flex items-center gap-1.5 disabled:opacity-50 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-gray-200 hover:bg-gray-700' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-100 shadow-sm'}`}
                            >
                                <CheckCheck size={15} /> Corregir
                            </button>
                            
                            <div className="flex gap-1">
                                <button type="button" onClick={() => handleAiTranslate('Inglés')} disabled={isAiLoading} className={`px-2.5 py-1.5 rounded-xl text-xs font-bold border transition-all ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'bg-white border-gray-300 hover:bg-gray-100 shadow-sm'}`} title="Traducir a Inglés">🇺🇸 EN</button>
                                <button type="button" onClick={() => handleAiTranslate('Francés')} disabled={isAiLoading} className={`px-2.5 py-1.5 rounded-xl text-xs font-bold border transition-all ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'bg-white border-gray-300 hover:bg-gray-100 shadow-sm'}`} title="Traducir a Francés">🇫🇷 FR</button>
                            </div>

                            {/* Dropdown de adjuntos multimedia */}
                            <div className="relative z-[9999]">
                                ...
                                {showPostAttachmentMenu && (
                                    <div className={`absolute bottom-full left-0 mb-2 w-52 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-2xl rounded-2xl p-2 flex flex-col gap-1 z-[9999] animate-in fade-in zoom-in-95 duration-200`}>
                                        <button type="button" onClick={() => { recPub ? stopPub() : startPub(); setShowPostAttachmentMenu(false); }} className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors w-full text-left ${recPub ? 'text-red-500 animate-pulse' : (isDarkMode ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-100 text-gray-700')}`}>
                                            {recPub ? <Square size={16} /> : <Mic size={16} />} {recPub ? 'Detener grabación...' : 'Nota de voz'}
                                        </button>
                                        <label className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold cursor-pointer transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-100 text-gray-700'}`}>
                                        ...
                                        <button type="button" onClick={() => { window.openGifPicker((url) => setPostImageUrl(url)); setShowPostAttachmentMenu(false); }} className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors w-full text-left ${isDarkMode ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-100 text-gray-700'}`}>
                                        ...
                                        <label className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold cursor-pointer transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-gray-200' : 'hover:bg-gray-100 text-gray-700'}`}>
                                            <PaperclipIcon size={16} /> Adjuntar documento (PDF/Word)
                    ...
                    {/* Parámetros de Entrega (Exclusivo de Tareas) */}
                    {postType === 'task' && (
                        <div className="flex flex-wrap gap-4 items-center justify-between bg-gray-50/80 dark:bg-gray-850/60 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 mt-2">
                            <div className="flex flex-wrap gap-4 items-center">
                                <div>
                                    <label className="text-xs font-bold text-gray-600 dark:text-gray-400 block mb-1">Fecha límite</label>
                                    <input type="date" value={taskDate} onChange={(e) => setTaskDate(e.target.value)} className={`${glassInput} !py-1.5 text-xs font-bold`} required />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-600 dark:text-gray-400 block mb-1">Hora límite</label>
                                    <input type="time" value={taskTime} onChange={(e) => setTaskTime(e.target.value)} className={`${glassInput} !py-1.5 text-xs font-bold`} required />
                                </div>
                    ...
                    <div className="flex justify-end items-center gap-2 pt-2 border-t border-gray-200/60 dark:border-gray-700/60 mt-1">
                        <button 
                            type="button" 
                            onClick={() => setIsFormExpanded(false)} 
